...
export const NonEmptyStringSchema = z
  ...
  .describe('Non-empty, trimmed string')
  .meta({ id: 'NonEmptyString' });
...
export const IdSchema = NonEmptyStringSchema
  .describe('Opaque identifier string (stable key; no semantics)')
  .meta({ id: 'Id', example: 'route_123' });
...
export const IsoDateTimeStringSchema = z
  ...
  .refine(
    (s) => {
      ...
      // Limit to 1800-01-01T00:00:00.000Z and 9999-12-31T23:59:59.999Z
  ...
  .describe('ISO 8601 datetime string, UTC only, arbitrary sub-second precision is supported.')
  .meta({ id: 'IsoDateTimeString', example: '2024-09-25T12:34:56Z', format: 'date-time' });
...
export const HttpUrlSchema = z
  .url()
  .refine((u) => /^https?:\/\//.test(u), 'HTTP/HTTPS URLs only')
  .describe('Absolute URL string starting with http:// or https://. Must be a valid HTTP or HTTPS URL.')
  .meta({ id: 'HttpUrl', example: 'https://agency.example.com' });
...
export const CoordinateSchema = z
  ...
  .meta({ id: 'Coordinate', example: { lat: 40.75, lng: -73.98 } });
...
export const IanaTimezoneSchema = z
  ...
  .meta({ id: 'IanaTimezone', example: 'America/New_York' });
...
export const ColorHexSchema = z
  ...
  .describe('CSS hex color: #RGB, #RGBA, #RRGGBB, or #RRGGBBAA (case-insensitive)')
  .meta({ id: 'ColorHex', example: '#336699' });
